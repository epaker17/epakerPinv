...
if (!address) { console.error('❌ Brak VITE_PINV_ADDRESS'); process.exit(1); }
...
if (!data?.result) { console.error('❌ Unexpected response', data); process.exit(1); }

let abi;
try { abi = JSON.parse(data.result); } catch (e) { console.error('❌ Could not parse ABI', e); process.exit(1); }
